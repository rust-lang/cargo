...
> [!NOTE]
> `--message-format=json` only controls Cargo and Rustc's output.
...
> [!NOTE]
> There is experimental nightly-only support for JSON output for tests,
...
> [!NOTE]
> Cargo defaults to prioritizing external tools in `$CARGO_HOME/bin`
