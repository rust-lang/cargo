...
///
/// The primary interface for this is [`crate::CargoPathExt::verify_file_layout`]
#[derive(Debug)]
...
impl LayoutTree {
    ...
    pub fn from_path(root_path: &Path) -> std::io::Result<Self> {
    ...
    pub fn matches_snapshot(&self, snapshot: &Self) -> bool {
        fn matches(node: &LayoutTreeNode, snap: &LayoutTreeNode) -> bool {
            ...
            let preprocess = |mut path: PathBuf| -> PathBuf {
                ...
                PathBuf::from(&adjust_canonicalization(&path))
            };
            ...
            for child in &node.children {
                ...
                for potential_match in snap
                    .children
                    .iter()
                    .filter(|p| preprocess(p.path.clone()) == preprocess(child.path.clone()))
                {
