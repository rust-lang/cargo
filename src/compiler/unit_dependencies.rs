            ...
            && !unit.kind.is_host()
        {
            ...
            let unit_dep = if dep_pkg_id.source_id().is_builtin() {
                ...
                if let Some(unit) = unit {
                    ...
                    // The resolve is target-independent, so may inject implicit dependencies that
                    // aren't actually needed for a particular target, such as if the target does
                    // not support std. The standard library resolve is the source of truth here.
                    continue;
                }
            } else {
                new_unit_dep(
...
fn compute_deps_doc(
    ...
    for (id, deps) in state.deps(unit, unit_for) {
        if id.source_id().is_builtin() {
            // TODO: Build-std for cargo doc is not yet implemented
            continue;
        }
