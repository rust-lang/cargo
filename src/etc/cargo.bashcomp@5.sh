...
_cargo(){
    ...
    prev="${COMP_WORDS[COMP_CWORD-1]}"
	  commands="bench build check clean clippy-preview doc fetch fix generate-lockfile git-checkout init install locate-project login metadata new owner package pkgid publish read-manifest run rustc rustdoc search test uninstall update verify-project version yank clippy fmt miri"
    ...
    local vcs='git hg none pijul fossil'
	  ...
	  local opt__locate_project="$opt_mani $opt_help"
