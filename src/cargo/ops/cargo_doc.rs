...
pub fn doc(ws: &Workspace<'_>, options: &DocOptions) -> CargoResult<()> {
    ...
    // any versioning. Therefore, we can end up with weird bugs and behaviours if we mix different
...
fn check_rustdoc_appropiate_versioning(
    ...
    let mut rustdoc_fingerprint_file = match File::open(RustDocTargetData::path_to_fingerprint(ws))
    {
        ...
        Err(_) => {
            ...
            match remove_doc_directory(ws) {
                // If the folder does not exist, we don't care since our goal was to
                // delete it.
                _ => (),
            }
...
/// Tries to remove the Workspace's `target/doc/` directory with all of it's contents
...
fn remove_doc_directory(ws: &Workspace<'_>) -> std::io::Result<()> {
    ...
    std::fs::remove_dir_all(&ws.root().join("target").join("doc")).into()
}
