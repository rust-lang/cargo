...
pub fn install(root: Option<&str>,
    ...
    let (pkg, source) = if source_id.is_git() {
        ...
        src.update()
            .chain_error(|| {
                human(format!("`{}` is not a crate root; specify a crate to install from \
                               crates.io, or use --path or --git to specify an alternate source",
                              path.display()))
            })?;
        ...
        select_pkg(map.load(source_id)?,
                   ...
                   config,
                   &mut |_| {
                       Err(human("must specify a crate to install from crates.io, or use --path \
                                  or --git to specify alternate source"))
                   })?
    };
    ...
    let compile =
        ops::compile_ws(&ws, Some(source), opts, Arc::new(DefaultExecutor)).chain_error(|| {
    ...
    for &(bin, src) in binaries.iter() {
        ...
        fs::copy(src, &dst).chain_error(|| {
                human(format!("failed to copy `{}` to `{}`", src.display(), dst.display()))
            })?;
    }
    ...
    for bin in to_install.iter() {
        ...
        fs::rename(&src, &dst).chain_error(|| {
                human(format!("failed to move `{}` to `{}`", src.display(), dst.display()))
            })?;
    ...
    config.shell()
        .warn(&format!("be sure to add `{}` to your PATH to be able to run the installed \
                        binaries",
                       dst.display()))?;
    Ok(())
}
            ...
            let vers = match vers {
                Some(v) => {
                    match v.parse::<Version>() {
                        Ok(v) => Some(format!("={}", v)),
                        Err(_) => {
                            let msg = format!("the `--vers` provided, `{}`, is not a valid \
                                               semver version\n\nhistorically Cargo treated this \
                                               as a semver version requirement accidentally\nand \
                                               will continue to do so, but this behavior will be \
                                               removed eventually",
                                              v);
...
pub fn uninstall(root: Option<&str>,
    ...
    let mut to_remove = Vec::new();
    {
        ...
        let bins = bins.iter()
            .map(|s| if s.ends_with(env::consts::EXE_SUFFIX) {
                s.to_string()
            } else {
