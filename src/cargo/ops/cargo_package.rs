...
pub fn package(ws: &Workspace<'_>, opts: &PackageOpts<'_>) -> CargoResult<Vec<FileLock>> {
    ...
    let mut local_reg = TmpRegistry::new(ws.gctx(), reg_dir, publish_reg)?;
    ...
    for (pkg, cli_features) in sorted_pkgs {
        ...
        if opts.list {
            ...
            let tarball = create_package(ws, &pkg, ar_files, Some(&local_reg))?;
    ...
    if opts.verify {
        for (pkg, opts, tarball) in &outputs {
            run_verify(ws, pkg, tarball, Some(&local_reg), opts)
...
fn local_deps(packages: impl Iterator<Item = (Package, CliFeatures)>) -> LocalDependencies {
    ...
    for (pkg, _features) in packages.values() {
        ...
        for dep in pkg.dependencies() {
            // Ignore dev-dependencies because they aren't needed for intra-workspace
            // lockfile generation or verification.
