...
fn clean_specs(
    ...
    if clean_ctx.gctx.cli_unstable().build_dir_new_layout {
        for pkg in packages {
            ...
            // Clean hashed example files, depinfo/debug files, and incremental compilation files.
            for target in pkg.targets() {
                if target.is_custom_build() {
                    continue;
                }
                ...
                for &mode in &[
                    ...
                    for (compile_kind, layout) in &layouts {
                        ...
                        if is_example {
                            let examples_dir = layout.build_dir().examples();
                            let examples_dir_glob = escape_glob_path(examples_dir)?;

                            let triple = target_data.short_name(compile_kind);
                            let (file_types, _) = target_data.info(*compile_kind).rustc_outputs(
                                mode,
                                target.kind(),
                                triple,
                                clean_ctx.gctx,
                            ...
                            // Clean hashed example artifacts and unhashed source files.
                            for file_type in file_types {
                                let hashed_name = file_type.output_filename(target, Some("*"));
                                clean_ctx.rm_rf_glob(
                                    &Path::new(&examples_dir_glob).join(&hashed_name),
                                )?;
                                // Also remove the unhashed source file. On Windows, uplifting
                                // creates hardlinks, and the source file may have a different
                                // name than the uplifted file (underscores vs hyphens).
                                let unhashed_name = file_type.output_filename(target, None);
                                clean_ctx.rm_rf(&examples_dir.join(&unhashed_name))?;
                            }

                            // Clean rustc-generated depinfo and debug files.
                            let mut glob_pattern = examples_dir_glob;
                            if !glob_pattern.ends_with(std::path::MAIN_SEPARATOR) {
                                glob_pattern.push(std::path::MAIN_SEPARATOR);
                            }
                            glob_pattern.push('*');
                            ...
                            clean_ctx.rm_rf_prefix_list(&glob_pattern, &paths)?;
                        }
