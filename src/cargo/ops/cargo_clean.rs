            ...
            for &mode in &[
                ...
                for (compile_kind, layout) in &layouts {
                    ...
                    for file_type in file_types {
                        ...
                        let hashed_dep_info = dir_glob.join(format!("{crate_name}-*.d"));
                        rm_rf_glob(&hashed_dep_info, config, &mut progress)?;
                        let unhashed_dep_info = dir.join(format!("{crate_name}.d"));
                        rm_rf(&unhashed_dep_info, config, &mut progress)?;
                        // Remove split-debuginfo files generated by rustc.
                        let split_debuginfo_obj = dir_glob.join(format!("{crate_name}.*.o"));
                        rm_rf_glob(&split_debuginfo_obj, config, &mut progress)?;
                        let split_debuginfo_dwo = dir_glob.join(format!("{crate_name}.*.dwo"));
