...
pub fn run(ws: &Workspace,
            ...
            match xs.len() {
                ...
                1 => {
                    ws.members()
                        ...
                        .ok_or_else(|| {
                            human(format!("package `{}` is not a member of the workspace", xs[0]))
                        })?
                }
    ...
    if bins.next().is_none() {
        match options.filter {
            CompileFilter::Everything => bail!("a bin target must be available for `cargo run`"),
            CompileFilter::Only { .. } => {
