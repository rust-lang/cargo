...
pub fn into_resolve(
    encoded_resolve: EncodableResolve,
    original: &str,
    ws: &Workspace<'_>,
) -> CargoResult<Resolve> {
    ...
    let mut version = match encoded_resolve.version {
    ...
    let packages = {
        let mut packages = encoded_resolve.package.unwrap_or_default();
        if let Some(root) = encoded_resolve.root {
    ...
    let mut metadata = encoded_resolve.metadata.unwrap_or_default();
    ...
    for pkg in encoded_resolve.patch.unused {
        let id = match pkg
            .source
            .as_deref()
            .or_else(|| get_source_id(&path_deps, &pkg))
