...
use crate::sources::source::{DynSource, QueryKind};
...
#[dynosaur::dynosaur(pub DynRegistry = dyn(box) Registry)]
...
impl<'gctx> PackageRegistry<'gctx> {
    ...
    pub fn add_preloaded(&mut self, source: Box<DynSource<'gctx>>) {
        self.add_source(source, Kind::Locked);
    }

    /// Adds a source to the registry.
    fn add_source(&self, source: Box<DynSource<'gctx>>, kind: Kind) {
    ...
    pub fn add_override(&mut self, source: Box<DynSource<'gctx>>) {
    ...
    pub fn patch(
        ...
        let unlocked_summaries = futures::executor::block_on(async {
    ...
    async fn query(
        ...
        match (override_summary, source) {
            ...
            (None, Some(source)) => {
                ...
                return source.query(dep, kind, callback).await.with_context(|| {
                    format!(
                        "failed to query {} for dependency `{}`",
                        source.source_id(),
                        dep.package_name()
                    )
                });
            ...
            (Some(override_summary), Some(source)) => {
                ...
                source.query(dep, kind, callback).await.with_context(|| {
                    format!(
                        "failed to query {} for dependency `{}`",
                        source.source_id(),
                        dep.package_name()
