...
//!         examples/
//!
//!         # Legacy deps directory, where tests and benchmarks are stored.
//!         # This is how older Cargo worked, and some tests use `current_exe`
//!         # to find binaries.
//!         deps/
//!
...
pub struct Layout {
    ...
    deps: PathBuf,
    /// The "legacy" deps directory: `$root/{debug,release}/deps`
    /// Cargo previously stored all artifacts here. Now, it only holds tests
    /// and benchmarks for backwards compatibility (because some of them use
    /// `current_exe` to discover binary executables).
    legacy_deps: PathBuf,
...
impl Layout {
    ...
    pub fn new(ws: &Workspace<'_>, triple: Option<&str>, dest: &str) -> CargoResult<Layout> {
        ...
        Ok(Layout {
            deps: root.join("deps"),
            legacy_deps: root_dest.join("deps"),
