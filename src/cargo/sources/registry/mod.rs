...
///
/// For general concepts of registries, see the [module-level documentation](crate::sources::registry).
#[dynosaur::dynosaur(DynRegistryData = dyn(box) RegistryData)]
...
impl<'gctx> RegistrySource<'gctx> {
    ...
    pub fn remote(
        ...
        let ops = if source_id.is_sparse() {
            DynRegistryData::new_box(http_remote::HttpRegistry::new(source_id, gctx, &name)?)
        } else {
            DynRegistryData::new_box(remote::RemoteRegistry::new(source_id, gctx, &name))
        };
...
impl<'gctx> Source for RegistrySource<'gctx> {
    ...
    fn download(&self, package: PackageId) -> CargoResult<MaybePackage> {
        let hash = futures::executor::block_on(self.index.hash(package, &*self.ops))?;
