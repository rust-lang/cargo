...
impl<'cfg> RegistryIndex<'cfg> {
    ...
    fn parse_registry_package(&mut self, line: &str) -> CargoResult<(Summary, bool)> {
        let RegistryPackage { name, vers, cksum, deps, features, yanked } =
    ...
    fn parse_registry_dependency(&self, dep: RegistryDependency) -> CargoResult<Dependency> {
        let RegistryDependency { name, req, features, optional, default_features, target, kind } =
            dep;
...
impl<'cfg> Registry for RegistryIndex<'cfg> {
    fn query(&mut self, dep: &Dependency) -> CargoResult<Vec<Summary>> {
        ...
        summaries.retain(|s| match self.source_id.precise() {
