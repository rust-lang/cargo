...
use crate::sources::registry::{DynRegistryData, LoadResponse, RegistryData};
...
impl<'gctx> RegistryIndex<'gctx> {
    ...
    pub async fn hash(&self, pkg: PackageId, load: &DynRegistryData<'_>) -> CargoResult<String> {
        ...
        'a: 'b,
    {
        ...
        impl<'a, 'b> Iterator for I<'a, 'b> {
            type Item = IndexSummary;

            fn next(&mut self) -> Option<Self::Item> {
                for (v, summary) in self.summaries.versions.iter().skip(self.i) {
                    self.i += 1;
    ...
    pub async fn is_yanked(&self, pkg: PackageId, load: &DynRegistryData<'_>) -> CargoResult<bool> {
        let req = OptVersionReq::lock_to_exact(pkg.version());
        let found = self
