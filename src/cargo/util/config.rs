...
impl Config {
    ...
    pub fn default() -> CargoResult<Config> {
        let shell = Shell::new();
        Ok(Config::new(shell, Config::cargo_dirs()))
    }
    ...
    pub fn cargo_dirs() -> CargoDirs {
        ...
        let executable_dir = base_dirs.executable_dir().unwrap();

        CargoDirs {
            home_dir: Filesystem::new(home_dir),
            current_dir: current_dir,
            cache_dir: Filesystem::new(cargo_dirs.cache_dir().to_path_buf()),
            config_dir: Filesystem::new(cargo_dirs.config_dir().to_path_buf()),
            data_dir: cargo_dirs.data_dir().to_path_buf(),
            executable_dir: executable_dir.to_path_buf(),
        }
    }

    /// The user's cargo home directory (OS-dependent) //FIXME? rename, wth?
    ...
    pub fn executable_path(&self) -> PathBuf {
        self.dirs.executable_dir.clone()
    }
    ...
    pub fn cwd(&self) -> &Path { &self.dirs.current_dir }
    ...
    pub fn load_values(&self) -> CargoResult<HashMap<String, ConfigValue>> {
        ...
        walk_tree(&self.dirs.current_dir, |path| {
...
fn cargo_home_dir(cwd: &Path) -> Option<PathBuf> {
    ...
    where F: FnMut(&Path) -> CargoResult<()>
{
    ...
    // information.
    let home = cargo_home_dir(pwd).ok_or_else(|| {
        format_err!("Cargo couldn't find your home directory. \
                     This probably means that $HOME was not set.")
    })?;
