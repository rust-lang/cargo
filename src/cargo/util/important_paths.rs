...
pub fn find_root_manifest_for_cwd(manifest_path: Option<String>) -> CliResult<Path> {
    manifest_path
        .map(|path| Ok(Path::new(path)))
        .unwrap_or_else(|| os::getcwd()
            .map_err(|_| CliError::new("Could not determine the current directory", 103))
        ...
        .and_then(|path| os::make_absolute(&path)
            .map_err(|_| CliError::new("Could not determine the absolute path of the manifest", 104)))
}
