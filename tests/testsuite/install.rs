...
[ERROR] failed to parse crate metadata at `[ROOT]/home/.cargo/.crates.toml`
  ...
  = caused by: TOML parse error at line 1, column 4
                 |
               1 | v1]
                 |    ^
...
[ERROR] failed to compile `foo v0.0.1 ([ROOT]/foo)`, intermediate artifacts can be found at `[ROOT]/foo/target`.
  ...
  = caused by: process didn't exit successfully: `rustc - --crate-name ___ --print=file-names --target nonexistent --crate-type bin --crate-type rlib --crate-type dylib --crate-type cdylib --crate-type staticlib --crate-type proc-macro --print=sysroot --print=split-debuginfo --print=crate-name --print=cfg -Wwarnings` ([EXIT_STATUS]: 1)
               --- stderr
               [ERROR] error loading target specification: could not find specification for target "nonexistent"
                 |
...
[ERROR] failed to compile `foo v0.1.0 ([ROOT]/foo)`, intermediate artifacts can be found at `[ROOT]/foo/target`.
       To reuse those artifacts with a future compilation, set the environment variable `CARGO_TARGET_DIR` to that path.
  |
  = caused by: rustc 1.92.0-nightly is not supported by the following package:
