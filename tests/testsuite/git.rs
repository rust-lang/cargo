...
[ERROR] failed to get `dep1` as a dependency of package `foo v0.5.0 ([ROOT]/foo)`
  ...
  = caused by: object not found - no match for id (887a18e6051ad0fcd3f24e75e302de8f1d22114c); class=Odb (9); code=NotFound (-3)
...
[UPDATING] git repository `[ROOTURL]/dep1`
[UPDATING] git submodule `[ROOTURL]/dep2`
[UPDATING] git submodule `https://127.0.0.1:42287/`
[ERROR] failed to get `dep1` as a dependency of package `foo v0.5.0 ([ROOT]/foo)`
  |
  = caused by: failed to load source for dependency `dep1`
  = caused by: Unable to update [ROOTURL]/dep1
  ...
  = caused by: failed to fetch submodule `bar` from https://127.0.0.1:42287/
