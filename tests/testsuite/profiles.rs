...
fn frame_pointers_force_on() {
    ...
    p.cargo("build --release -v")
        .with_stderr_data(str![[r#"
[COMPILING] foo v0.1.0 ([ROOT]/foo)
[RUNNING] `rustc [..] -C force-frame-pointers=yes [..]`
[FINISHED] `release` profile [optimized] target(s) in [ELAPSED]s
...
fn frame_pointers_force_off() {
    ...
    p.cargo("build --release -v")
        .with_stderr_data(str![[r#"
[COMPILING] foo v0.1.0 ([ROOT]/foo)
[RUNNING] `rustc [..] -C force-frame-pointers=no [..]`
[FINISHED] `release` profile [optimized] target(s) in [ELAPSED]s
...
Caused by:
  `frame-pointers` setting of `invalid` is not a valid setting, must be `"force-on"` or `"force-off"`.
