...
fn no_config_gated() {
    ...
    p.cargo("build --no-config")
        .with_status(101)
        .with_stderr(
            "\
[ERROR] the `--no-config` flag is unstable, and only available \
...
",
    ...
    p.cargo("build --no-config")
        ...
        .with_stderr(
            "\
[ERROR] the `--no-config` flag is unstable, pass `-Z unstable-options` to enable it
See [..]
",
        )
        .run();
