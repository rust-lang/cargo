...
[HELP] there is a feature `feat2` with a similar name
...
// FIX: old, bad summary_cache can cause bar not updated with the_feat
...
// TODO: we should add some other message to help user about "?/" problems
//
#[cargo_test]
fn bad_deferred() {
    // A complex chain that requires deferring enabling the feature due to
...
[HELP] there is a feature `feat` with a similar name
