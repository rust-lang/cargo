...
/// Updating a package whose new versions conflict with the preferred version of an unrelated
/// dependency edge.
///
/// ```
/// foo
/// ├── b 1.0.0
/// │   └── shared >=1, <3 # Locked to 1.1.0
/// ├── bar 1.0.0 # The package being updated
/// │   └── shared 1 # Locked to 1.1.0
/// └── d 1.0.0
///     └── shared 2 # Locked to 2.0.0
/// ```
///
/// The new versions of `bar` require `shared 1.5`, so updating `bar` also requires moving `b`'s
/// edge from `shared 1.1.0` to another compatible version (`1.5.0` or `2.0.0`).
#[cargo_test]
fn update_with_conflicting_preferred_dependency() {
    Package::new("shared", "1.1.0").publish();
    Package::new("b", "1.0.0")
        .dep("shared", ">=1, <3")
        .publish();
    Package::new("bar", "1.0.0").dep("shared", "1").publish();
...
"#]])
    ...
    // Later, `shared 2.0.0` is published and `d`, which requires it, is added to the project.
    Package::new("shared", "2.0.0").publish();
    Package::new("d", "1.0.0").dep("shared", "2").publish();
    p.change_file(
        "Cargo.toml",
        r#"
            [package]
...
version = "1.0.0"
source = "registry+https://github.com/rust-lang/crates.io-index"
checksum = "9a6dee46f8e5b467aadbeedcb893996536e8d2aff74c93cc1bf109aecb2c0cd8"
dependencies = [
 "shared 2.0.0",
]
...
version = "2.0.0"
source = "registry+https://github.com/rust-lang/crates.io-index"
checksum = "5b7e5a3a91c4925f90efaa88a5a28b5b5883a51145b7382a8b7d6be77d5b2f62"

"##]],
    );

    // Later still, `shared 1.5.0` is published, and new `bar` versions that require it.
    Package::new("shared", "1.5.0").publish();
    for patch in 1..=5 {
        Package::new("bar", &format!("1.0.{patch}"))
            .dep("shared", "1.5")
            .publish();
    }
