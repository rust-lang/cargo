...
#[cargo_test]
fn inherited_detailed_dependencies() {
...
fn error_no_root_workspace() {
    ...
    p.cargo("build")
        ...
        .with_stderr(
            "\
[ERROR] root of a workspace inferred but wasn't a root: [..]/Cargo.toml
",
        )
        .run();
}
...
Caused by:
  failed to get dependency `foo`, [workspace.dependencies] does not exist
",
        )
        .run();
}
