...
[RUNNING] \
                    ...
                    .with_stdout("
running 1 test
test bench_hello ... bench: [..] 0 ns/iter \
                                  (+/- 0)
...
fn bench_tarname() {
    ...
    let prj = project("foo")
        .file("Cargo.toml",
            ...
            authors = []
        "#)
        .file("benches/bin1.rs",
              r#"
            #![feature(test)]
            extern crate test;
            #[bench] fn run1(_ben: &mut test::Bencher) { }"#)
        .file("benches/bin2.rs",
              r#"
            #![feature(test)]
            extern crate test;
...
[FINISHED] release \
                                           [optimized] target(s) in [..]
[RUNNING] \
                                           `[..]target[/]release[/]deps[/]foo-[..][EXE] hello \
                                           --bench`",
                                          url = p.url()))
                    .with_stdout("
running 1 test
...
fn many_similar_names() {
    ...
    let p = project("foo")
        ...
        .file("src/main.rs",
            ...
            extern \
               crate test;
            fn main() {}
            #[bench] fn bin_bench(_b: &mut \
               test::Bencher) { foo::foo() }
        ")
        .file("benches/foo.rs",
              r#"
            #![feature(test)]
    ...
    assert!(output.contains("test bin_bench"),
            "bin_bench missing\n{}",
            output);
    assert!(output.contains("test lib_bench"),
            "lib_bench missing\n{}",
            output);
    assert!(output.contains("test bench_bench"),
            "bench_bench missing\n{}",
            output);
}
...
[COMPILING] foo v0.5.0 ({})
[FINISHED] \
                                                   release [optimized] target(s) in [..]
\
                                                   [RUNNING] \
                                                   target[/]release[/]deps[/]foo-[..][EXE]
\
                                                   thread '[..]' panicked at 'assertion \
                                                   failed: `(left == right)` (left: \
                                                   `\"hello\"`, right: `\"nope\"`)', \
                                                   src[/]foo.rs:14
[..]
",
...
[FINISHED] release \
                                           [optimized] target(s) in [..]
[RUNNING] \
                                           target[/]release[/]deps[/]baz-[..][EXE]
[RUNNING] \
                                           target[/]release[/]deps[/]foo-[..][EXE]",
                                          p.url()))
                    .with_stdout("
running 1 test
test bin_bench ... bench: [..] 0 ns/iter (+/- \
                                  0)
...
\
                                  running 1 test
test lib_bench ... bench: [..] 0 ns/iter (+/- \
                                  0)
...
[FINISHED] release [optimized] \
                                           target(s) in [..]
[RUNNING] \
                                           target[/]release[/]deps[/]bar-[..][EXE]",
                                          dir = p.url()))
                    .with_stdout("
running 1 test
test bar_bench ... bench: [..] 0 ns/iter (+/- \
                                  0)
...
[RUNNING] \
                    ...
                    .with_stdout("
running 1 test
test external_bench ... bench: [..] 0 ns/iter \
                                  (+/- 0)
...
[FINISHED] release \
                                           [optimized] target(s) in [..]
[RUNNING] \
                                           target[/]release[/]deps[/]external-[..][EXE]
\
...
fn dont_run_examples() {
    ...
    let p = project("foo")
        ...
        .file("src/lib.rs",
              r#"
        "#)
        .file("examples/dont-run-me-i-will-fail.rs",
              r#"
            fn main() { panic!("Examples should not be run by 'cargo test'"); }
        "#);
    assert_that(p.cargo_process("bench"), execs().with_status(0));
}
...
[RUNNING] \
                                           target[/]release[/]deps[/]bench-[..][EXE]
[RUNNING] \
                                           target[/]release[/]deps[/]syntax-[..][EXE]",
                                          dir = p.url()))
                    .with_stdout("
running 1 test
...
\
                                  running 1 test
test foo_bench ... bench: [..] 0 ns/iter (+/- \
                                  0)
...
fn lib_with_standard_name2() {
    ...
    let p = project("foo")
        ...
        .file("src/main.rs",
            ...
            #[bench]
            fn \
               bench(_b: &mut test::Bencher) { syntax::foo() }
        ");
...
[FINISHED] release [optimized] \
                                           target(s) in [..]
[RUNNING] \
                                           `{dir}[/]target[/]release[/]deps[/]testb1-[..][EXE] \
                                           --bench`
[RUNNING] \
                                           `{dir}[/]target[/]release[/]deps[/]testbench-[..\
                                           ][EXE] --bench`",
                                          dir = p.root().display(),
                                          url = p.url()))
                    .with_stdout("
running 1 test
test bench_bench2 ... bench: [..] 0 ns/iter \
                                  (+/- 0)
...
[RUNNING] \
                                  target[/]debug[/]deps[/]b-[..][EXE]")
                    .with_stdout("
running 1 test
test foo ... ok

test result: ok. 1 passed; 0 \
                                  failed; 0 ignored; 0 measured
...
[RUNNING] target[/]release[/]deps[/]bbaz-[..][EXE]")
                    .with_stdout_contains("
running 1 test
test bench_baz ... bench:           \
                                           0 ns/iter (+/- 0)

test result: ok. 0 passed; 0 \
                                           failed; 0 ignored; 1 measured
")
                    .with_stderr_contains("\
[RUNNING] target[/]release[/]deps[/]bbar-[..][EXE]")
                    .with_stdout_contains("
running 1 test
test bench_bar ... bench:           \
                                           0 ns/iter (+/- 0)
