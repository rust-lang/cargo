...
",
                                                            dir = p.url())));
    assert_that(&p.root().join("target/package/foo-0.0.1.crate"),
                existing_file());
...
fn package_lib_with_bin() {
    ...
    assert_that(p.cargo_process("package").arg("-v"), execs().with_status(0));
}
...
",
                                                            dir = p.url())));
    assert_that(&p.root().join("target/package/nested-0.0.1.crate"),
                existing_file());
    assert_that(p.cargo("package").arg("-l"),
                execs().with_status(0).with_stdout("\
Cargo.toml
...
"));
    ...
    for f in ar.entries().unwrap() {
        ...
        let fname = f.header().path_bytes();
        let fname = &*fname;
        assert!(fname == b"nested-0.0.1/Cargo.toml" || fname == b"nested-0.0.1/src/main.rs",
                "unexpected filename: {:?}",
                f.header().path())
    }
}
...
fn repackage_on_source_change() {
    ...
    assert_that(p.cargo_process("package"), execs().with_status(0));
...
",
    ...
    let entry_paths = entries.map(|entry| entry.unwrap().path().unwrap().into_owned())
        .collect::<Vec<PathBuf>>();
    assert_that(&entry_paths,
...
error: failed to prepare local package for \
                                           uploading
...
to proceed despite this, pass the `--allow-dirty` \
                                  flag
"));
}
