...
fn parent_doesnt_point_to_child() {
    ...
    assert_that(p.cargo("build").cwd(p.root().join("bar")),
                execs()
                    .with_status(101)
                    .with_stderr("\
error: current package believes it's in a workspace when \
                                  it's not:
current: [..]Cargo.toml
...
fn two_roots() {
    ...
    assert_that(p.cargo("build"),
                execs()
                    .with_status(101)
                    .with_stderr("\
error: multiple workspace roots found in the same \
                                  workspace:
  [..]
  [..]
"));
...
this may be fixable by adding `bar` to the \
...
fn virtual_build() {
    ...
    assert_that(p.cargo("build"),
                execs()
                    .with_status(101)
                    .with_stderr("\
error: manifest path `[..]` is a virtual manifest, but this \
                                  command requires running against an actual package in this \
                                  workspace
"));
}
...
fn new_warns_you_this_will_not_work() {
    ...
    assert_that(p.cargo("new").arg("--lib").arg("bar").env("USER", "foo"),
                execs()
                    .with_status(0)
                    .with_stderr("\
warning: compiling this new crate may not work due to \
                                  invalid workspace configuration

current package believes \
                                  it's in a workspace when it's not:
current: [..]
workspace: \
                                  [..]

this may be fixable by ensuring that this crate is \
                                  depended on by the workspace root: [..]
[CREATED] library \
                                  `bar` project
"));
}
...
fn you_cannot_generate_lockfile_for_empty_workspaces() {
    ...
    assert_that(p.cargo("update"),
                execs()
                    .with_status(101)
                    .with_stderr("\
error: you can't generate a lockfile for an empty \
                                  workspace.
"));
}
...
fn test_path_dependency_under_member() {
    ...
    assert_that(&p.root().join("foo/bar/Cargo.lock"),
