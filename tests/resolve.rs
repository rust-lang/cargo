...
fn resolve<R: Registry>(pkg: PackageId,
                        deps: Vec<Dependency>,
                        registry: &mut R)
                        -> CargoResult<Vec<PackageId>> {
    let summary = Summary::new(pkg.clone(), deps, HashMap::new()).unwrap();
    let method = Method::Everything;
    Ok(resolver::resolve(&[(summary, method)], &[], registry)
        ?
        .iter()
        .map(|p| p.clone())
        .collect())
...
fn test_resolving_empty_dependency_list() {
    let res = resolve(pkg_id("root"), Vec::new(), &mut registry(vec![])).unwrap();
...
fn test_resolving_multiple_deps() {
    ...
    let res = resolve(pkg_id("root"), vec![dep("foo"), dep("baz")], &mut reg).unwrap();
...
fn test_resolving_common_transitive_deps() {
    ...
    let res = resolve(pkg_id("root"), vec![dep("foo"), dep("bar")], &mut reg).unwrap();
...
fn test_resolving_with_dev_deps() {
    let mut reg = registry(vec![pkg!("foo" => ["bar", dep_kind("baz", Development)]),
...
fn resolving_with_many_versions() {
    let mut reg = registry(vec![pkg!(("foo", "1.0.1")), pkg!(("foo", "1.0.2"))]);
...
fn resolving_with_specific_version() {
    ...
    let res = resolve(pkg_id("root"), vec![dep_req("foo", "=1.0.1")], &mut reg).unwrap();
...
fn test_resolving_maximum_version_with_transitive_deps() {
    let mut reg = registry(vec![pkg!(("util", "1.2.2")),
                                pkg!(("util", "1.0.0")),
                                pkg!(("util", "1.1.1")),
...
fn resolving_incompat_versions() {
    let mut reg = registry(vec![pkg!(("foo", "1.0.1")),
                                pkg!(("foo", "1.0.2")),
                                pkg!("bar" => [dep_req("foo", "=1.0.2")])]);

    assert!(resolve(pkg_id("root"),
...
fn resolving_backtrack() {
    let mut reg = registry(vec![pkg!(("foo", "1.0.2") => [dep("bar")]),
                                pkg!(("foo", "1.0.1") => [dep("baz")]),
                                pkg!("bar" => [dep_req("foo", "=2.0.2")]),
                                pkg!("baz")]);

    let res = resolve(pkg_id("root"), vec![dep_req("foo", "^1")], &mut reg).unwrap();

    assert_that(&res,
                contains(names(&[("root", "1.0.0"), ("foo", "1.0.1"), ("baz", "1.0.0")])));
}

#[test]
fn resolving_allows_multiple_compatible_versions() {
    let mut reg = registry(vec![pkg!(("foo", "1.0.0")),
                                pkg!(("foo", "2.0.0")),
                                pkg!(("foo", "0.1.0")),
                                pkg!(("foo", "0.2.0")),
...
fn resolving_with_deep_backtracking() {
    let mut reg = registry(vec![pkg!(("foo", "1.0.1") => [dep_req("bar", "1")]),
...
fn resolving_but_no_exists() {
    ...
    let res = resolve(pkg_id("root"), vec![dep_req("foo", "1")], &mut reg);
    assert!(res.is_err());
...
#[test]
fn resolving_cycle() {
    let mut reg = registry(vec![pkg!("foo" => ["foo"])]);

    let _ = resolve(pkg_id("root"), vec![dep_req("foo", "1")], &mut reg);
}
...
fn hard_equality() {
    ...
    assert_that(&res,
                contains(names(&[("root", "1.0.0"), ("foo", "1.0.0"), ("bar", "1.0.0")])));
}
