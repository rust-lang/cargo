...
test!(dep_with_changed_submodule {
    ...
    git_project.process("git").args(["fetch"]).cwd(git_project.root().join("src")).exec_with_output().assert();
    git_project.process("git").args(["reset", "--hard", "origin/master"]).cwd(git_project.root().join("src")).exec_with_output().assert();
    git_project.process("git").args(["add", "."]).exec_with_output().assert();
    git_project.process("git").args(["commit", "-m", "test"]).exec_with_output()
               .assert();
