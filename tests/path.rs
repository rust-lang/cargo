...
fn nested_deps_recompile() {
    ...
    File::create(&p.root().join("src/foo.rs"))
        .unwrap()
        .write_all(br#"
        fn main() {}
    "#)
...
[ERROR] failed to update path override \
                                  `[..]../whoa-this-does-not-exist` (defined in `[..]`)
...
fn invalid_path_dep_in_workspace_with_lockfile() {
    ...
    assert_that(p.cargo("build"),
                execs()
                    .with_status(101)
                    .with_stderr("\
error: no matching package named `bar` found (required by \
                                  `foo`)
location searched: [..]
version required: *
"));
