...
",
    ...
    assert_that(&p.root().join("target/doc"), existing_dir());
    assert_that(&p.root().join("target/doc/foo/index.html"), existing_file());
    assert_that(&p.root().join("target/doc/bar/index.html"),
...
fn doc_only_bin() {
    ...
    assert_that(p.cargo_process("doc").arg("-v"), execs().with_status(0));
...
fn doc_lib_bin_same_name() {
    ...
    assert_that(p.cargo_process("doc"),
                execs()
                    .with_status(101)
                    .with_stderr("\
[ERROR] cannot document a package where a library and a \
                                  binary have the same name. Consider renaming one or marking \
...
fn doc_target() {
    ...
    assert_that(p.cargo_process("doc").arg("--target").arg(TARGET).arg("--verbose"),
                execs().with_status(0));
    assert_that(&p.root().join(&format!("target/{}/doc", TARGET)),
                existing_dir());
    assert_that(&p.root().join(&format!("target/{}/doc/foo/index.html", TARGET)),
                existing_file());
}
...
fn output_not_captured() {
    ...
    let output = p.cargo_process("doc")
        .exec_with_output()
        .err()
        .unwrap()
        .output
    ...
    assert!(stderr.contains("unknown start of token"),
            "no message\n{}",
            stderr);
}
...
fn document_only_lib() {
    let p = project("foo")
        ...
        .file("src/bin/bar.rs",
            ...
            fn main() { foo(); }
        "#);
    assert_that(p.cargo_process("doc").arg("--lib"), execs().with_status(0));
    assert_that(&p.root().join("target/doc/foo/index.html"), existing_file());
}
