...
fn cargo_compile_simple_git_dep() {
    ...
    let project = project.file("Cargo.toml",
              &format!(r#"
            [project]
...
fn cargo_compile_with_meta_package() {
    let git_project = git::new("meta-dep", |project| {
            project.file("dep1/Cargo.toml",
                ...
                name = "dep1"
            "#)
                .file("dep1/src/dep1.rs",
                      r#"
                pub fn hello() -> &'static str {
                    "this is dep1"
                }
            "#)
                .file("dep2/Cargo.toml",
                      r#"
                [project]
    ...
    let p = project("parent")
        ...
        "#,
                       ...
                       git_project.url()))
        .file("src/parent.rs",
              &main_file(r#""{} {}", dep1::hello(), dep2::hello()"#,
                         &["dep1", "dep2"]));
...
\
                                           Caused by:
  invalid url `{}`: relative URL without \
                                           a base
",
                                          url)));
}
...
fn recompilation() {
    ...
    let p = project("foo")
        ...
        "#,
                       git_project.url()))
        .file("src/foo.rs", &main_file(r#""{:?}", bar::bar()"#, &["bar"]));
    ...
    assert_that(p.cargo("update"),
                execs().with_stderr(&format!("[UPDATING] git repository `{}`", git_project.url())));
...
fn dep_with_changed_submodule() {
    ...
    sub.sync().unwrap();
    {
        ...
        subrepo.remote_add_fetch("origin", "refs/heads/*:refs/heads/*")
            .unwrap();
        subrepo.remote_set_url("origin", &git_project3.url().to_string())
            .unwrap();
...
fn update_ambiguous() {
    ...
    assert_that(p.cargo("update")
                ...
                execs()
                    .with_status(101)
                    .with_stderr("\
[ERROR] There are multiple `foo` packages in your project, \
                                  and the specification `foo` is ambiguous.
Please re-run this \
                                  command with `-p <spec>` where `<spec>` is one of the \
                                  following:
  foo:0.[..].0
  foo:0.[..].0
"));
